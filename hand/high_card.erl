	  ...
	  Flush ->
	    case is_straight(Flush#hand.value) of
	      ...
	      Straight ->
	        ...
	        #hand{rank = {?STRAIGHT_FLUSH, Kind}, value = Flush#hand.value, high = build_card(Kind, Suit)}
	    end
  end.
...
is_four_kind(Cards) ->
	...
	if
		...
		true ->
			...
			#hand{rank = {?FOUR_KIND, FourKind#card_group.kind}, value = FourKind#card_group.value, kicker = kicker_cards(Cards, FourKind#card_group.value, 1)}
	end.
...
is_three_kind(Cards) ->
	...
	if
		erlang:length(Repeats) == 1 ->
			[ThreeKind | _] = Repeats,
			#hand{rank = {?THREE_KIND, ThreeKind#card_group.kind}, value = ThreeKind#card_group.value, kicker = kicker_cards(Cards, ThreeKind#card_group.value, 2)};
		true ->
			false
	end.
...
is_two_pair(Cards) ->
	...
	if
		erlang:length(Pairs) >= 2 ->
			...
			Value = Major#card_group.value ++ Minor#card_group.value,
			#hand{rank = {?TWO_PAIR, [Major#card_group.kind, Minor#card_group.kind]}, value = Value, kicker = kicker_cards(Cards, Value, 1)};
		true ->
			false
	end.
...
is_one_pair(Cards) ->
	...
	if
		erlang:length(Pairs) == 1 ->
			...
			#hand{rank = {?ONE_PAIR, First#card.kind}, value = Pair#card_group.value, kicker = kicker_cards(Cards, Pair#card_group.value, 3)};
		true ->
			false
	end.
...
is_high_card(Cards) ->
	...
	#hand{rank = {?HIGH_CARD, Highest#card.kind}, value = [Highest], kicker = kicker_cards(Cards, [Highest], 4)}.
...
high_card(Cards) when is_list(Cards) and erlang:length(Cards) > 7 ->
	throw("7 cards per hand allowed");
